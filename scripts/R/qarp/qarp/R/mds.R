...
qarp.read.cachedtsv <- function(filename, sep='\t', cache=TRUE) {
    ...
    if (cache) {
        e <- try({
                  suupressWarnings(load(paste0(base, ".Rda")))
              }, silent=TRUE)
...
##' @param invert Draw one of the groups of profiles below the hozontal axis.
##' @param xlim limitations of x-axis, if applicable.   This allows you to zoom in on particular areas of interest.
##' @author Chris Warth
##' @export
qarp.plotProfile <- function(mat, aVec, showsplices=TRUE, invert=TRUE, xlim=NULL) {
    gene <- attr(mat, "gene")
    ...
    ## If invert is TRUE, invert the control profils so they show below the horizontal axis,
    ## its just another eway of looking at the grpah and some people find easier to distinguish
    ## between the two groups that way.
    if (invert)
