...
if (length(opt$args) == 0) {
   # If the user did not supply any arguments (running interactivey iside RStudio)
   # Then prompt for the names of datasets.
...
results = morris.maplot(datasets, mincount=opt$options$mincount, normalization=opt$options$normalization, group=opt$options$group)
